...
class SavannahView {
  ...
  rightTranslationActions(translationEl, rightAnswer) {
    ...
    setTimeout(() => {
      this.removeHighlight(translationEl, rightAnswer);
    }, DELAY);
    this.gameStatistics.appendCorrectAnswer(
      this.randomEngWord,
      this.model.correctAnswer,
      this.model.currentWordAudio,
    );
  }
  ...
  wrongTranslationActions(translationEl, wrongAnswer, rightAnswer) {
    ...
    setTimeout(() => {
      this.removeHighlight(translationEl, wrongAnswer);
    }, DELAY);
    setTimeout(() => {
    ...
    this.gameStatistics.appendWrongAnswer(
      this.randomEngWord,
      this.model.correctAnswer,
      this.model.currentWordAudio,
    );
  ...
  renderTranslation() {
    ...
    fourAnswersWordsArr.forEach((item, index) => {
      spanArr.push(
        `<span class="translation translation-${index + 1}"><span class="keyboard-num">${
          index + 1
        }</span>${item}</span>`,
      );
    });
